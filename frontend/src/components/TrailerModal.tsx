...
export default function TrailerModal({ movieId, movieTitle, movieYear, onClose }: TrailerModalProps) {
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 20 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 8, letterSpacing: 4, textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)', textAlign: 'center', fontWeight: 500 }}>
                Official Trailer{movieYear ? ` — ${movieYear}` : ''}
              </div>
            </div>
          </div>
        </div>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderTop: 'none', padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
          <button
            onClick={onClose}
